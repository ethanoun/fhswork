...
rs_path = os.path.join(data_path, 'race_sex.json')
...
def encode(tests:list):
    ...
    id_list = [key for key in data_json(rs_path)]
    ...
    rs_json = data_json(rs_path)
    result = {}
    for i in dummy_list:
        resp = rs_json[i]

        #determines sex of patient by arbitary number
