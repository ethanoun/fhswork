...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
mri_path = os.path.abspath(os.path.join(parent, 'mri.json'))
...
def encode(tests:list):
    ...
    #id_list = [key for key in data_json(mri_path)] # for testing when json can't be loaded from the cloud
    ...
    for i,stream in enumerate(streams):
        for value in stream:
            sets[i].add(value['_id'].split('-')[0]) # i is always 0, value is in exp. FHS_ES99878 format
...
'''if test_paths[idx] not in resp[-1]:
                resp.append({test_paths[idx]:1})
            if i not in j:
                resp[-1][test_paths[idx]] = -1'''
