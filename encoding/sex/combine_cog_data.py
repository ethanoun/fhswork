...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
sex_path = os.path.abspath(os.path.join(parent, 'race_sex.json'))
...
def encode(tests:list):
    ...
    id_list = util.query_ids(client,'student-rs')
    id_list = [key for key in data_json(sex_path)] # for testing when json can't be loaded from the cloud
    ...
    rs_json = data_json(sex_path)
    result = {}
    for i in dummy_list:
        resp = rs_json[i]
        #print(resp)

        # determines if the participant is male or female based on the value ascribed to the 'sex' key
        resp['is_male'] = str(abs(2 - int(resp['sex'])))
        resp['is_female'] = str(abs(1 - int(resp['sex'])))
