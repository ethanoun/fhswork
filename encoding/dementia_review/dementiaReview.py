...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
dr_path = os.path.abspath(os.path.join(parent, 'dementia_review.json'))
...
def encode(tests:list):
    ...
    id_list = util.query_ids(client,'student-dr')
    #id_list = [key for key in data_json(dr_path)] # for testing when json can't be loaded from the cloud
    ...
    dementia_review_json = data_json(dr_path)
    result = {}
    for i in dummy_list:
        resp = dementia_review_json[i]
        #print(resp)
