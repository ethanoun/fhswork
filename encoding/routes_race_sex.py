...
race_sex_bp = Blueprint(
    'race_sex_bp', __name__,
)


@race_sex_bp.route("/encoding/race_sex",methods=['GET','POST'])
def encode_race_sex():
    ...
    if (request.method == 'POST') & bool(tests):
        ...
        return redirect(url_for('home_bp.download', functype='encoding', testtype = 'race_sex', **data))

    submitted = request.args.getlist('f_args')
    return '''<form method="post">
<h2> Select encoding data for Race/Sex </h2>
