...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
educ_path = os.path.abspath(os.path.join(parent, 'education.json'))
...
def encode(tests:list):
    ...
    #id_list = util.query_ids(client,'student-educ')
    id_list = [key for key in data_json(educ_path)] # for testing when json can't be loaded from the cloud
