...
@dnp_bp.route("/encoding/dnp",methods=['GET','POST'])
def encode_dnp():
    ...
    if (request.method == 'POST') & bool(tests):
        ...
        return redirect(url_for('home_bp.download', functype='encoding', testtype = 'dnp', **data))

    submitted = request.args.getlist('f_args')
    return '''<form method="post">
<h2> Select encoding data for DNP </h2>
