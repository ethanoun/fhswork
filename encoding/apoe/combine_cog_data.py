...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
apoe_path = os.path.abspath(os.path.join(parent, 'apoe.json'))
...
def encode(tests:list):
    ...
    id_list = util.query_ids(client,'student-apoe')
    #id_list = [key for key in data_json(apoe_path)] # for testing when json can't be loaded from the cloud
