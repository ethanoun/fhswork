...
@mri_bp.route("/encoding/mri",methods=['GET','POST'])
def encode_mri():
    ...
    if (request.method == 'POST') & bool(tests):
        ...
        return redirect(url_for('home_bp.download', functype='encoding', testtype = 'mri', **data))

    submitted = request.args.getlist('f_args')
    return '''<form method="post">
<h2> Select encoding data for MRI </h2>
