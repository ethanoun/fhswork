...
@tau_bp.route("/encoding/tau",methods=['GET','POST'])
def encode_tau():
    ...
    if (request.method == 'POST') & bool(tests):
        ...
        return redirect(url_for('home_bp.download', functype='encoding', testtype = 'tau', **data))

    submitted = request.args.getlist('f_args')
    return '''<form method="post">
<h2> Select encoding data for TAU </h2>
