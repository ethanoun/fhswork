...
@pib_bp.route("/encoding/pib",methods=['GET','POST'])
def encode_pib():
    ...
    if (request.method == 'POST') & bool(tests):
        ...
        return redirect(url_for('home_bp.download', functype='encoding', testtype = 'pib', **data))

    submitted = request.args.getlist('f_args')
    return '''<form method="post">
<h2> Select encoding data for PIB </h2>
