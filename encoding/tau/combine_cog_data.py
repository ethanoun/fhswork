...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
tau_path = os.path.abspath(os.path.join(parent, 'tau.json'))
...
def encode(tests:list):
    ...
    test_paths = []
    for i in tests:
        path = 'student-' + i   # 'student-tau'
        test_paths.append(path) # ['student-tau']

    auth = ('student','fhs1234') # tuple
    util.make_client(auth, None) 
    client = util.get_client()

    id_list = util.query_ids(client,'student-tau')
    id_list = [key for key in data_json(tau_path)] # for testing when json can't be loaded from the cloud
