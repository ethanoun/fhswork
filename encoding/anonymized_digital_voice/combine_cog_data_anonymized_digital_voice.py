...
for (root, dirs, files) in os.walk(parent):
    if root.split('\\')[-1] == 'data':
        parent = root
        break
adv_path = os.path.abspath(os.path.join(parent, 'anonymized_digital_voice.json'))
#data path runs through each folder by going from the bottom which would be encoding and contiously moving up to the main drive
#anon_dvoice_path joins the path with the data folder and has it access the json file
